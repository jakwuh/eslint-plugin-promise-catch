                ...
                if (catchNode) {
                    ...
                    if (isBlockStatement(body)) {
                        if (body.body.length) {
                            ...
                            if (isReturnStatement(firstNode) &&
                                isCallExpression(firstNode.argument) &&
                                isMemberExpression(firstNode.argument.callee)) {
                                var promiseNode = firstNode.argument;

                                if (isIdentifier(promiseNode.callee.object, 'Promise') &&
                                    isIdentifier(promiseNode.callee.property, 'reject') &&
                                    identifiersEql(promiseNode.arguments[0], error)) {
                                    return context.report(promiseNode, 'Only rejecting Promise with error inside catch block is no-op.');
