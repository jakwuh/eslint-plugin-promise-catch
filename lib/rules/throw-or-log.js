module.exports = {
    ...
    create: function (context) {
        ...
        function isLogger(node, error) {
            ...
            var isValidArgs = (node.arguments || []).find(function (arg) {
                return identifiersEql(arg, error);
            });
            ...
            CallExpression: function (node) {
                ...
                if (catchNode) {
                    ...
                    if (isBlockStatement(body)) {
                        if (body.body.length &&
                            isThrowStatement(body.body[0]) &&
                            identifiersEql(body.body[0].argument, error)) {
                            return context.report(body.body[0], 'Only throwing error inside catch block is no-op.');
                        }
                    } else {
                        body = catchNode;
                    }
                    ...
                    traverser.traverse(body, {
                        enter: function (node, parent) {
                            ...
                            if (isThrowStatement(node) && identifiersEql(node.argument, error) && sameScope(body, node)) {
